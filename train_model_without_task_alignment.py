...
from src.datasets.seam_data_module import *
...
def main(args):
    ...
    collate_fn = SEAMCollator(args.modalities)
    ...
    args.num_labels = config.num_labels_in_task[args.task_name]
    txt_logger.log(f'total_labels: {args.num_labels}')
    ...
    if args.dataset_name=='seam':
        ...
        if (args.wandb_log_name is not None) and (args.exe_mode=='train'):
            loggers_list.append(loggers.WandbLogger(save_dir=args.log_base_dir, 
                            ...
                            project='Multimodal-Domain-Adaptation-TS'))
...
def start_training(args, txt_logger, loggers=None):
    ...
    txt_logger.log(f"\n\n$$$$$$$$$ Start training $$$$$$$$$\n\n")
    dataModule = SEAMDataModule(args)
    
    if(args.is_student_training):
        model = StudentTrainer(hparams=args)
    else:
        model = TeacherTrainer(hparams=args)
