...
class StudentTrainer(pl.LightningModule):
    def __init__(self,
        ...
        if(hparams.resume_teacher_checkpoint_filename is not None):
        ...
        self.domain_classifier = Classifier(self.hparams.indi_modality_embedding_size, 
                                        self.hparams.num_domains)
    ...
    def eval_step(self, batch, batch_idx, pre_log_tag, optimizer_idx=0):
        ...
        if(optimizer_idx==0):
            ...
            kd_loss = self.kd_logits(student_mm_embed, teacher_mm_embed)

            same_domain_label = torch.zeros(batch['domain_label'].size(0), 1)
            same_domain_label = same_domain_label.type_as(batch['domain_label'])
            same_domain_label = same_domain_label.squeeze(dim=1)

            domain_classifier_output = self.domain_classifier(student_mm_embed)
            # _, pred_domain_label = torch.max(F.softmax(domain_classifier_output, dim=1), 1)
            pred_domain_label = domain_classifier_output
            # print('$$$$$$$$', pred_domain_label.shape, same_domain_label.shape)
            # print(same_domain_label)
            # print(pred_domain_label)
            domain_adversarial_loss = self.loss_fn(pred_domain_label, same_domain_label)

            loss = kd_loss + domain_adversarial_loss
            ...
            domain_adversarial_loss = self.loss_fn(pred_domain_label, batch['domain_label'])
            loss = domain_adversarial_loss
            self.log(f'{pre_log_tag}_adv_loss', loss)
    ...
    def configure_optimizers(self):
        ...
        optimizer_domain_classifier = torch.optim.AdamW(self.domain_classifier.parameters(), lr=self.hparams.learning_rate)
        ...
        lr_scheduler_domain_classifier = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer_domain_classifier,
                                                                        T_0=self.hparams.cycle_length,
                                                                        T_mult=self.hparams.cycle_mul)                                                                        
        return [optimizer_mm_encoder, optimizer_domain_classifier], [lr_scheduler_mm_encoder, lr_scheduler_domain_classifier]
