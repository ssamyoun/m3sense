...
from src.models.Student_Trainer import *
...
class StudentTrainerFinetune(pl.LightningModule):
    def __init__(self,
                 hparams):

        super(StudentTrainerFinetune, self).__init__()

        self.hparams.update(vars(hparams))

        self.student_model = StudentTrainer(hparams=hparams)
        if(hparams.is_load_student_trainer):
            student_resume_path = f'{hparams.model_save_base_dir}/{hparams.resume_student_checkpoint_filename}'
            self.student_model, _ = load_model(self.student_model, student_resume_path, strict_load=False)
        ...
        print('$$$$$$$$$$', config.num_labels_in_task[self.hparams.student_target_task_name])
        # build learning model
        self.mm_encoder = self.student_model.mm_encoder
        self.har_classifier = Classifier(self.hparams.indi_modality_embedding_size, 
                                        config.num_labels_in_task[self.hparams.student_target_task_name])
    ...
    def forward(self, batch):
        if(self.hparams.is_encoder_finetune):
    ...
    def configure_optimizers(self):
        if(self.hparams.is_encoder_finetune):
            model_params = list(self.mm_encoder.parameters()) + list(self.har_classifier.parameters())
        else:
            model_params = list(self.har_classifier.parameters())
