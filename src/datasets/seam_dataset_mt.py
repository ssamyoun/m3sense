...
class SEAMDatasetMT(Dataset):
    ...
    def prepare_data_element(self, idx, skip_frame_len_dict=None):
        ...
        if(int(label)==2 and self.hparams.task_name=='Anxiety'):
            data['label'] = 1    
        else:
        ...
        for task_name in self.hparams.task_list:
            data[f'{task_name}_label'] = config.domain_labels[task_name]
...
class SEAMCollator:
    ...
    def __init__(self, modalities, task_list):
    ...
    def __call__(self, batch):
        ...
        for task_name in self.task_list:
            data[f'{task_name}_label'] = torch.tensor([batch[bin][f'{task_name}_label'] for bin in range(batch_size)],
                                    dtype=torch.float)
        return data